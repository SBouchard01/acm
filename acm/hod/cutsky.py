...
class BaseCutskyCatalog(ABC):
    ...
    def apply_angular_mask(
        ...
        if custom_mask_path is None:
            ...
            self.catalog["HPX"], is_in_photo = self.is_in_photometric_region(
                self.catalog["RA"], self.catalog["DEC"], region=region
            )
            ...
            mask_in_desi = is_in_desi_footprint(
                mask_ra, mask_dec, release=release, program=program, npasses=npasses
            )
            _, mask_in_photo = self.is_in_photometric_region(
                mask_ra, mask_dec, region=region
            )
    ...
    def apply_radial_mask(
        self, nz_filename: str, shape_only: bool = False, dz_new: float = 0.002
    ) -> None:
        """
    ...
    def compute_nz(
        self, zedges: np.ndarray | None = None
    ...
    def is_in_photometric_region(
        self, ra: np.ndarray, dec: np.ndarray, region: str
    ) -> tuple[np.ndarray, np.ndarray]:
        """
    ...
    def add_columns_fiberassign(catalog, seed: int = 0) -> None:
        ...
        np.random.seed(seed)
        catalog["TARGETID"] = np.random.permutation(
            np.arange(1, catalog.size + 1)
        ) + catalog.mpicomm.rank * (10**6)  # to make iot unique across processes
    ...
    @CurrentMPIComm.enable
    def run_FA_for_single_tracer(
        self,
        release="Y3",
        program="dark",
        npasses=7,
        use_sky_targets=False,
        seed=None,
        preload_sky_targets=False,
        plot_output=True,
        mpicomm=None,
        mpiroot=0,
    ):
        ...
        """
        from mockfactory.desi import (
            apply_fiber_assignment,
            build_tiles_for_fa,
        ...
        nbar2 = (
            240 if tr_toadd[1] == "ELG_HIP" else 310 if tr_toadd[1] == "QSO" else 610
        )
        seed1, seed2 = rng.randint(0, 2**32 - 1, size=2)
        cutsky_1 = RandomCutskyCatalog(
            rarange=(self.catalog["RA"].min(), self.catalog["RA"].max()),
            decrange=(self.catalog["DEC"].min(), self.catalog["DEC"].max()),
            drange=(self.catalog["Distance"].min(), self.catalog["Distance"].max()),
            nbar=nbar1,
            seed=seed1,
            mpicomm=mpicomm,
        )
        cutsky_2 = RandomCutskyCatalog(
            rarange=(self.catalog["RA"].min(), self.catalog["RA"].max()),
            decrange=(self.catalog["DEC"].min(), self.catalog["DEC"].max()),
            drange=(self.catalog["Distance"].min(), self.catalog["Distance"].max()),
            nbar=nbar2,
        ...
        cutsky_for_fa = Catalog.concatenate(
            [self.catalog[cutsky_1.columns()], cutsky_1, cutsky_2]
        )
        ...
        tiles = build_tiles_for_fa(
            release_tile_path=f"/global/cfs/cdirs/desi/survey/catalogs/{release}/LSS/tiles-{program.upper()}.fits",
            program=program,
            npasses=npasses,
        )
        ...
        fht = fitsio.read_header(
            f"/global/cfs/cdirs/desi/target/fiberassign/tiles/trunk/{ts[:3]}/fiberassign-{ts}.fits.gz"
        )
        rundate = fht["RUNDATE"]
        ...
        apply_fiber_assignment(
            ...
            columns_for_fa,
            mpicomm,
            use_sky_targets=use_sky_targets,
        )
        ...
        # Summarize and plot
        ra, dec = (
            cutsky_for_fa.cget("RA", mpiroot=0),
            cutsky_for_fa.cget("DEC", mpiroot=0),
        )
        numobs, available = (
            cutsky_for_fa.cget("NUMOBS", mpiroot=0),
            cutsky_for_fa.cget("AVAILABLE", mpiroot=0),
        )
        obs_pass, comp_weight = (
            cutsky_for_fa.cget("OBS_PASS", mpiroot=0),
        ...
        if plot_output & (mpicomm.rank == 0):
            ...
            logger.info(
                f"In percentage: Observed: {(numobs >= 1).sum() / ra.size:2.2%} -- Available: {available.sum() / ra.size:2.2%}"
            )
            values, counts = np.unique(comp_weight, return_counts=True)
            logger.info(
                f"Sanity check for completeness weight: {available.sum() - (numobs >= 1).sum()} avialable unobserved targets and {np.nansum([(val - 1) * count for val, count in zip(values, counts)])} from completeness counts"
            )
            ...
            fig.tight_layout()
            fig.savefig(
                f"fiberasignment_{npasses}npasses.png",
                facecolor="w",
                bbox_inches="tight",
                pad_inches=0.2,
                dpi=400,
    ...
    def save(self, filename: str) -> None:
        ...
        catalog_gathered = {}
        for key in self.catalog.keys():
            catalog_gathered[key] = mpy.gather(
                self.catalog[key], mpicomm=self.mpicomm, mpiroot=self.mpiroot
            )
...
class CutskyHOD(BaseCutskyCatalog):
    ...
    def __init__(
        ...
        **kwargs: dict
        ...
        if len(zranges) != len(snapshots):
            raise ValueError(
                "Number of redshift ranges must match number of snapshots."
            )
        ...
        if self.load_existing_hod:
            ...
            if DM_DICT is None:
                DM_DICT = lookup_registry_path(
                    "Abacus.yaml", self.tracer, self.DM_DICT_simtype
                )
    ...
    def _sample_hod(
        ...
        if self.mpicomm.rank == self.mpiroot:
            ...
            vel = np.c_[hod_dict["VX"], hod_dict["VY"], hod_dict["VZ"]].astype(
                np.float32
            )
        else:
            pos = None
    ...
    def load_hod(
        self, mock_path=None, zsnap: float = 0.5, cosmo_idx: int = 0, phase_idx: int = 0
    ):
        """
        ...
        for i, (zsnap, zranges) in enumerate(zip(self.snapshots, self.zranges)):
            ...
            if self.load_existing_hod:
                box_positions, box_velocities = self.load_hod(
                    mock_path=existing_hod_path, zsnap=zsnap
                )
            else:
                ball = self.balls[i]
            ...
            pos_min, pos_max = self.get_reference_borders(
                zranges, region=region, release=release, custom_xyz_file=custom_xyz_file
            )
            ...
            cutsky_shell = self.box_to_cutsky(
                box=box, zmin=zranges[0], zmax=zranges[1], zrsd=zsnap, apply_rsd=True
            )
    ...
    def get_box_shifts(self, pos_min: np.ndarray, pos_max: np.ndarray) -> list:
        """
    ...
    def get_box_replications(
        ...
        for shift in shifts:
            temp_pos, temp_vel = self.get_pos_within_borders(
                position + shift, velocity, pos_min, pos_max, target_nbar
            )
    ...
    def box_to_cutsky(
        ...
        cutsky["Distance"], cutsky["RA"], cutsky["DEC"] = mockfactory.cartesian_to_sky(
            box[pos]
        )
...
class CutskyRandoms(BaseCutskyCatalog):
    ...
    def __init__(
        self,
        rarange: tuple = (0.0, 360.0),
        decrange: tuple = (-90.0, 90.0),
