...
def _default_sample_function(void_cat, column="R"):
    ...
    for i in range(len(limits) - 1):
        ...
        try:
            toret.append(
                void_cat.loc[mask, ["x", "y", "z", "R", "dtfe", "sphericity"]].values
            )
        except KeyError:
...
class DTVoid(BaseEstimator):
    ...
    def _galcat_to_voidcat(
        ...
        while max_r > cpy_range:
            cpy_range *= 1.1
            if full_cat:
                voids, gal_dtfe = get_void_catalog_full(
                    data_positions,
                    periodic=periodic,
                    box_min=self.boxmin,
            ...
            if periodic:
                box_mask = (
                    (voids[:, :3] > 0) & (voids[:, :3] < np.asarray(self.boxsize))
                ).all(axis=1)
        ...
        if self.full_catalog:
            voids = pd.DataFrame(
                voids, columns=["x", "y", "z", "R", "vol", "dtfe", "area"]
            )
            voids["sphericity"] = (
                np.pi ** (1 / 3) * (6 * voids["vol"]) ** (2 / 3) / voids["area"]
            )
        else:
    ...
    def sample_data_correlation(self, data_positions, **kwargs):
        ...
        if self.has_randoms:
            ...
            randoms_positions = np.array_split(
                kwargs.pop("randoms_positions"), nsplits, axis=0
            )
        ...
        for i, sample in enumerate(self.samples):
            if self.has_randoms:
                split_rands = np.array_split(
                    self.void_randoms[i][:, :3], nsplits, axis=0
                )
            R1R2 = None
            result = 0
    ...
    def plot_one_point(self):
        if self.full_catalog:
            fig, ax = plt.subplots(
                ...
                nrows=1,
                ncols=self.samples[0][:, 3:].shape[1],
            )
        else:
